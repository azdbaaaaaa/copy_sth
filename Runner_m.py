...
from BLL import BexcelPeport as excel
from COMMON import operaterXML as om
...
#得到xml配置接口信息和http请求实体
def get_api():
    return om.getXML('',Mhttpbase.BaseHttp())
...
class TestInterfaceCase(unittest,TestCase):
        ...
        if self.index ==1 :
            if gm[self.index]["method"] == "POST":
                ...
                if gm[self.index]["login"] != "0":
                    go.PARAMS[go.LOGIN_KEY] = go.LOGIN_VALUE #登录成功后返回来的值，可能是token,userid等
                if gm[self.index]["method"] == "POST":
                ...
                if gm[self.index]["method"] == "GET":
                    go.PARAMS = hp.str_post_param(gm[self.index]["param"][0], go.PARAMS) #把登录成功后的token,userid合并到请求参数中
    ...
    def run_case(runner):
        ...
        case_list = ast.literal_eval(case_list) #字符chuan类型的list转换为list
        if len(case_list) == False:
            temp_case = gm
            for index in range (1, len(temp_case)):
                ...
                for i in case_list:
                    for j in range(1, len(gm)):
                        ...
                        runner.run(test_suite)
                        #记录结果
                        mresult.info.append(json.dumps(resultInfo(MresultDetail.resultInfo(), t_id=gm[j]["id"], t_name=gm[j]["name"],
    ...
    if __name__ == '__main__':
        ...
        #测试报告
        workbook = xlswriter.Workbook('report.xlsx')
        worksheet = workbook.add_worksheet("测试总况")
        worksheet2 = workbook.add_worksheet("测试详情")
        data = json.loads(json.dumps(result(mresult, test_data=str(sum_time) + "毫秒", test_sum=go.CASE_TOTAL, test_fail=go.ERROR_NUM, test_version="v2.2", test_pl="python2.7",
